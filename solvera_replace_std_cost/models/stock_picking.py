...
class StockPicking(models.Model):
    ...
    def update_stock_surbaya(self):
        for i in self:
             for line in i.move_line_ids_without_package:
                not_product = line.location_dest_id.quant_ids.filtered(lambda x:x.product_id.id == line.product_id.id).sudo()
                if line.location_id.branch_id.id == 2:
    ...
    def update_stock_bali(self):
        for i in self:
             for line in i.move_line_ids_without_package:
                not_product = line.location_dest_id.quant_ids.filtered(lambda x:x.product_id.id == line.product_id.id).sudo()
                if line.location_id.branch_id.id == 3:
                    if line.location_dest_id.branch_id.id == 1:
                        temp = 0
                        for lines in not_product:
                            get_quant = lines.quantity
    ...
    def update_stock(self):
        for i in self:
            for line in i.move_line_ids_without_package:
                not_product = line.location_dest_id.quant_ids.filtered(lambda x:x.product_id.id == line.product_id.id).sudo()
                if line.location_id.branch_id.id == 1:
                    self.update_stock_jakarta()
                              
                ### From Surabaya
                elif line.location_id.branch_id.id == 2:
                    self.update_stock_surbaya()
                
                ### From bali
                elif line.location_id.branch_id.id == 3:
                    self.update_stock_bali()

                else:
                    continue
                    
    def update_valuation_jakarta(self):
                    ...
                    if this.branch_id.id == 1:
                        ...
                        if line.product_id == lines.product_id:
                            if vl_object.account_move_id:
                                ...
                                move_id.sudo().post()
    def update_valuation_surabaya(self):
        move_obj=self.env['account.move']
        for this in self:
            for line in this.move_lines:
                vl_object= self.env['stock.valuation.layer'].sudo().search([('stock_move_id','=',line._origin.id)])
                for lines in line.stock_valuation_layer_ids:
                    if this.branch_id.id == 2:
    ...
    def update_valuation_bali(self):
        move_obj=self.env['account.move']
        for this in self:
            for line in this.move_lines:
                vl_object= self.env['stock.valuation.layer'].sudo().search([('stock_move_id','=',line._origin.id)])
                for lines in line.stock_valuation_layer_ids:
                    if this.branch_id.id == 3:
    ...
    def update_valuation(self):
        ...
        for this in self:
            for line in this.move_lines:
                for lines in line.stock_valuation_layer_ids:
                    if this.branch_id.id == 1:
                        self.update_valuation_jakarta()
                                
                    elif this.branch_id.id == 2:
                        self.update_valuation_surabaya()
                                
                    elif this.branch_id.id == 3:
                        self.update_valuation_bali()
                    else:
                        continue


    def update_valuation_return_po(self):
        move_obj=self.env['account.move']
        for this in self:
            for line in this.move_lines:
                vl_object= self.env['stock.valuation.layer'].sudo().search([('stock_move_id','=',line._origin.id)])
    ...
    def _action_done(self):
        ...
        if self.picking_type_code == 'outgoing':
            if self.purchase_id:
                ...
                self.update_valuation_return_po()
            else:
