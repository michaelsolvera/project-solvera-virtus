    ...
    def product_price_update_before_done(self, forced_qty=None):
        ...
        for move in self.filtered(lambda move: move.location_id.usage in ('supplier', 'production') and move.product_id.cost_method in ('average', 'last','branch')):
            if move.picking_code == 'incoming':
                    ...
                    if move.product_id.cost_method == 'branch':
                        if self.branch_id.id == 1 :
                            if float_is_zero(product_tot_qty_available_branch, precision_rounding=rounding):
                                new_std_price_jakarta = move._get_price_unit()
                                ...
                                qty_done = move.product_uom._compute_quantity(move.quantity_done, move.product_id.uom_id)
                                qty = forced_qty or qty_done
                                new_std_price_jakarta = ((amount_unit * product_tot_qty_available_branch) + (move._get_price_unit() * qty)) / (product_tot_qty_available_branch + qty_done)
                        elif self.branch_id.id == 2 :
                            if float_is_zero(product_tot_qty_available_branch, precision_rounding=rounding):
                                new_std_price_surabaya = move._get_price_unit()
                            else:
                                amount_unit = std_price_update.get(
                                    (move.company_id.id, move.product_id.id)) or move.product_id.standard_price_surabaya
                                qty_done = move.product_uom._compute_quantity(move.quantity_done, move.product_id.uom_id)
                                qty = forced_qty or qty_done
                                new_std_price_surabaya = ((amount_unit * product_tot_qty_available_branch) + (move._get_price_unit() * qty)) / (product_tot_qty_available_branch + qty_done)
                        elif self.branch_id.id == 3 :
                            if float_is_zero(product_tot_qty_available_branch, precision_rounding=rounding):
                                new_std_price_bali = move._get_price_unit()
                                ...
                                qty_done = move.product_uom._compute_quantity(move.quantity_done, move.product_id.uom_id)
                                qty = forced_qty or qty_done
                                new_std_price_bali= ((amount_unit * product_tot_qty_available_branch) + (move._get_price_unit() * qty)) / (product_tot_qty_available_branch + qty_done)
                        else:
                            amount_unit = (std_price_update.get(
                                (move.company_id.id, move.product_id.id)) or move.product_id.standard_price)
                            qty_done = move.product_uom._compute_quantity(move.quantity_done, move.product_id.uom_id)
                ...
                if move.product_id.cost_method == 'branch':
                    if self.branch_id.id == 1:
                        move.product_id.sudo().write({'standard_price_jakarta': new_std_price_jakarta})
                    elif self.branch_id.id == 2:
                        move.product_id.sudo().write({'standard_price_surabaya': new_std_price_surabaya})
                    elif self.branch_id.id == 3:
                        move.product_id.sudo().write({'standard_price_bali': new_std_price_bali})
                    else:
