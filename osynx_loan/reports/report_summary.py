...
class ReportSummary(models.AbstractModel):
    ...
    def get_actual_dividend(self,date_to):
        ...
        total_penalty = sum(
            r.amount for r in self.env['loan.payment'].search([
                ('state', '=', 'validate'),
                ('date', '<=', date_to),
                ('payment_type_code', 'in', ['EXPIRE_LOAN','LATE_CONTRIBUTION']),
    ...
    def get_summary_receivable(self,docs):
        ...
        total_payments = sum(r.amount for r in self.env['loan.payment'].search([
            ('state', '=', 'validate'),
            ('date', '<=', docs.date_to),
            ('payment_type_code', 'in', ['INTEREST', 'PRINCIPAL', 'EXPIRED_LOAN','LATE_CONTRIBUTION']),
        ]))
    ...
    def get_summary_loanable(self,docs):
        total_contribution = sum(r.amount for r in self.env['loan.payment'].search([
            ('state', '=', 'validate'),
            ('date', '<=', docs.date_to),
        ...
        total_payments = sum(r.amount for r in self.env['loan.payment'].search([
            ('state', '=', 'validate'),
            ('date', '<=', docs.date_to),
            ('payment_type_code', 'in', ['INTEREST', 'PRINCIPAL', 'EXPIRE']),
        ]))
