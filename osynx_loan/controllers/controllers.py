...
class LoanPaymentPortal(CustomerPortal):
    def _prepare_portal_layout_values(self):
        ...
        payment_count = request.env['loan.payment'].search(domain)
        values.update({
    ...
    def portal_my_loan_payments(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, search=None,
                                search_in='all', groupby='none', **kw):
        values = self._prepare_portal_layout_values()
        loan_payment_sudo = request.env['loan.payment'].sudo()
        ...
        searchbar_filters = {
            ...
            'interest': {'label': _('Interest Payment'), 'domain': [("payment_type_code", "=", 'INTEREST')]},
            'principal': {'label': _('Principal Payment'), 'domain': [("payment_type_code", "=", 'PRINCIPAL')]},
            'contribution': {'label': _('Monthly Contribution'), 'domain': [("payment_type_code", "=", 'CONTRIBUTION')]},
            'expire_loan_penalty': {'label': _('Expire Loan Penalty'), 'domain': [("payment_type_code", "=", 'EXPIRE_LOAN')]},
            'late_contribution_penalty': {'label': _('Late Contribution Penalty'), 'domain': [("payment_type_code", "=", 'LATE_CONTRIBUTION')]},
            'membership': {'label': _('Membership Fee'), 'domain': [("payment_type_code", "=", 'MEMBERSHIP')]},
            'transaction': {'label': _('Transaction Fee'), 'domain': [("payment_type_code", "=", 'TRANSACTION')]},
    ...
    def portal_my_loan_payment(self, payment_id=None, access_token=None, **kw):
        try:
            loan_payment_sudo = self._document_check_access('loan.payment', payment_id, access_token)
    ...
    def submit_payment(self, **kwargs):
        ...
        account_ids = request.env['member.account'].sudo().search([('partner_id','=',default_partner_id.id)])
        # member_ids = request.env['res.partner'].sudo().search([])
        member_ids = account_ids.mapped('partner_id')
        loan_ids = request.env['loan.account'].sudo().search([
            ('state','not in',['draft','paid']),
            ('guarantor_id','in',account_ids.ids)
        ])
        penalty_ids = request.env['loan.penalty'].sudo().search([
            ('state', 'not in', ['validate']),
        ])
        payment_type_ids = request.env['loan.payment.type'].sudo().search([])
...
class PaymentForm(Controller):
    ...
    def contribution_form(self, redirect=None, **kw):
        ...
        payment_obj = request.env['loan.payment']
