    ...
    def create_lab_test(self):
        ...
        for browse_record in browse_records:
            ...
            if res_ids:
                ...
                action = imd.sudo().xmlid_to_object('hospital_management.action_medical_lab_tree')
                list_view_id = imd.sudo().xmlid_to_res_id('hospital_management.medical_lab_tree_view')
                form_view_id  =  imd.sudo().xmlid_to_res_id('hospital_management.medical_lab_form_view')
                result = {
                                'name': action.name,
                                'help': action.help,
