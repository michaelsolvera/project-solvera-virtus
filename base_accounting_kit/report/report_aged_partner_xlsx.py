...
class ReportAgedPartnerBalance(models.AbstractModel):
    _name = 'report.base_accounting_kit.report_aged_partner_balance_xlsx'
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move,
        ...
        query = '''
                SELECT DISTINCT ON (res_partner.display_name) res_partner.display_name as name,l.partner_id, ru.login as login, team.name as team, l.date as date
        ...
        for partner in partners:
            ...
            values = {}
            undue_amt = 0.0
            values['salesperson'] = partner.get('login')
            values['team'] = partner.get('team')
            if partner[
            ...
            if partner['partner_id']:
                browsed_partner = self.env['res.partner'].browse(
                    partner['partner_id'])
                values['name'] = browsed_partner.display_name 
                # and len(
                #     browsed_partner.display_name) >= 45 and browsed_partner.display_name[
                #                                     0:40] + '...' or browsed_partner.display_name
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        sheet.merge_range('A2:L3', 'Aged Partner Balance', format1)
        row = 5
        col = 0
        ...
        if data['form']['result_selection'] == 'customer':
            ...
            account_type = ['payable', 'receivable']
        date_from = data['form'].get('date_from', time.strftime('%d-%m-%y'))
        target_move = data['form'].get('target_move', 'all')
        movelines, total, dummy = self._get_partner_move_lines(account_type,
        ...
        sheet.merge_range(row, col+3, row, col+6, date, date_format)
        row += 1
        ...
        sheet.write(row, col+3, "Sales Person", format5)
        sheet.write(row, col+4, "Sales Team", format5)
        sheet.write(row, col+5, "Not Due", format5)
        sheet.write(row, col+6, form['4']['name'], format5)
        sheet.write(row, col+7, form['3']['name'], format5)
        sheet.write(row, col+8, form['2']['name'], format5)
        sheet.write(row, col+9, form['1']['name'], format5)
        sheet.write(row, col+10, form['0']['name'], format5)
        sheet.write(row, col+11, "Total", format5)
        ...
        if total:
            ...
            sheet.write(row, col + 5,
                        total[6] and int(total[6])or '',
                        format2)
            sheet.write(row, col + 6,
                        total[4] and int(total[4])or '',
                        format2)
            sheet.write(row, col + 7,
                        total[3] and int(total[3])or '',
                        format2)
            sheet.write(row, col + 8,
                        total[2] and int(total[2])or '',
                        format2)
            sheet.write(row, col + 9,
                        total[1] and int(total[1])or '',
                        format2)
            sheet.write(row, col + 10,
                        total[0] and int(total[0])or '',
                        format2)
            sheet.write(row, col + 11,
                        total[5] and int(total[5])or '',
                        format2)
        ...
        for partner in movelines:
            ...
            sheet.write(row, col + 3,
                        partner['salesperson'] and (partner['salesperson'])or '',
                        format2)
            sheet.write(row, col + 4,
                        partner['team'] and (partner['team'])or '',
                        format2)
            sheet.write(row, col + 5,
                        partner['direction'] and int(partner['direction'])or '',
                        format2)
            sheet.write(row, col + 6,
                        partner['4'] and int(partner['4']) or '',
                        format2)
            sheet.write(row, col + 7,
                        partner['3'] and int(partner['3'])or '',
                        format2)
            sheet.write(row, col + 8,
                        partner['2'] and int(partner['2'])or '',
                        format2)
            sheet.write(row, col + 9,
                        partner['1'] and int(partner['1'])or '',
                        format2)
            sheet.write(row, col + 10,
                        partner['0'] and int(partner['0'])or '',
                        format2)
            sheet.write(row, col + 11,
                        partner['total'] and int(partner['total'])or '',
                        format2)
            row += 1
